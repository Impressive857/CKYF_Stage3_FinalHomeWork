...
void navigation::Node::robot_navigation_cbfn(const info_interfaces::msg::Robot::SharedPtr robot_info)
{
    ...
    if (robot_info->our_robot_real_pos.x == m_last_real_x && robot_info->our_robot_real_pos.y == m_last_real_y && !m_should_stop) {
        ...
        my_serial::password_receive_t password_receive;
        if (m_my_serial.read_from_port(&password_receive, sizeof(password_receive))) {
            m_password.data = password_receive.password;
            RCLCPP_INFO(get_logger(), "password got:%ld", m_password.data);
